	...
	popover.initialize = function() {
		...
		$member_select.removeClass('custom-select').selectize({
            ...
            placeholder: ([ 'OWNER', 'ADMINISTRATOR' ].indexOf(project.pivot.role) > -1
                ? __('popovers.manage_project_board_members.enter_name_username_or_email')
                : __('popovers.manage_project_board_members.enter_name_or_username')
            ),
            ...
            render: {
                option: function(item, escape) {
                    ...
                    return (
                        '<div class="selectize-item + is-user ' + (item.is_selected ? 'is-selected' : '') + ' ' + (item.is_disabled ? 'is-disabled' : '') + '">' +
