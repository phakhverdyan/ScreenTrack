            ...
            $('body').on('click', '.article-delete', function (event) {
                ...
                modals.confirm_action({
                    ...
                    confirm: function (callback) {
                        request({
                            method: 'GET',
                            url: '/blog_articles/' + article_id + '/delete',
                        }, function (response) {
                            if (response.error) {
...
@section('content')
    ...
    <form id="article-search-form">
        <div class="row mb-5">
            ...
            <div class="col-md-4">
                <a href="/nexus/blog/articles/create" class="btn btn-primary float-right">Add Article</a>
            </div>
        </div>
    </form>
    <div>
                ...
                <div data-id="{{ $article->id }}" class="row mb-5 article-item">
                    ...
                    <div class="col-md-6 ">
                        ...
                        <div class="float-right">
                            <a href="/nexus/blog/articles/{{ $article->id }}/edit" class="btn btn-primary">Edit</a>
                            <a data-id="{{ $article->id }}" href="" class="btn btn-primary article-delete ml-1">Delete</a>
                        </div>
                    </div>
                </div>
            @empty
                <div class="text-center">We don't have any Blog's articles yet :(</div>
            @endforelse
        </div>
    </div>
@endsection
