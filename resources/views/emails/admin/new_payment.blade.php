	...
	<table align="center" width="600" style="border-collapse: collapse; background: #fff; border-radius: 15px;">
		...
		<tr>
			<td style="padding: 0 20px 30px 20px;">
				<p>{{ $payment->payee_user->title }}</p>
				<p>Amount: ${{ $payment->original_amount }}</p>
				<p>Return: ${{ $payment->return_amount }}</p>
				@if ($payment->subscription)
					<p>
						<strong>Subscription:</strong>
						${{ $payment->subscription->price }}/{{ $payment->subscription->period }}
					</p>
				@endif
				<p>
					<strong>Influencer Email:</strong>
					{{ $payment->payee_user->email }}
				</p>
				<p>
					<strong>Influencer Locality:</strong>
					{{ $payment->payee_user->locality ? $payment->payee_user->locality->address : 'Undisclosed' }}
				</p>
				<p>
					<a href="{{ url($payment->payee_user->slug) }}" target="_blank">Go to Influencer PayPage</a>
				</p>
			</td>
		</tr>
	</table>
