                    ...
                    <div class="tab-pane fade show active">
                        @if ($tracking_days->count() > 0 || $milestones->count() > 0)
                            ...
                            @if ($milestones->count() > 0)
                                <table class="w-100 {{ $tracking_days->count() > 0 ? 'mt-3' : '' }}">
                                    <thead>
                                        <th class="px-2">{!! __('dashboard/earnings.date') !!}</th>
                                        <th class="px-2">{!! __('dashboard/earnings.client') !!}</th>
                                        <th class="px-2">{!! __('dashboard/earnings.project') !!}</th>
                                    ...
                                    <tbody>
                                        @foreach ($milestones as $milestone)
                                            <tr>
                                                ...
                                                <td class="px-2">
                                                    <a href="#" class="d-flex align-items-center" onclick="event.preventDefault(); slideups.user_profile({ user_id: parseInt('{{ $milestone->project->owner_user->id }}'), default_tab: 'contract' });">
                                                        <img src="{{ $milestone->project->owner_user->image->urls->tiny }}" style="width: 30px; height: 30px; border-radius: 50%;">
                                                        <span class="ml-1">{{ $milestone->project->owner_user->title }}</span>
                                                    </a>
                                                </td>
                                                <td class="px-2">
                                                ...
                                                <td class="px-2">${{ number_format($milestone->amount, 2) }}</td>
                                                <td class="px-2">
                            ...
                            @endif
                        @else
                            <div class="text-center">{!! __('dashboard/earnings.no_work_in_escrow') !!}</div>
                        @endif
                    </div>
                </div>
            </div>
