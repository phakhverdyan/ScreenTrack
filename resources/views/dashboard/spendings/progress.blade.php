            ...
            <div class="col-md-9">
                ...
                <div class="tab-content">
                    <div class="tab-pane fade show active">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($tracking_days as $tracking_day)
                                        <tr>
                                            ...
                                            <td class="px-2 text-center">
                                                ${{ number_format($tracking_day->average_hourly_rate, 2) }}/h
                                            </td>
                                            <td class="px-2 text-center">
                                                ${{ number_format(floor($tracking_day->total_amount * 100) / 100, 2) }}
                                            </td>
                                            <td class="px-2">
                        ...
                        @else
                            <div class="text-center">{!! __('dashboard/spendings.no_work_in_progress') !!}</div>
                        @endif
                    </div>
                </div>
            </div>
                ...
                <form method="get">
                    <div class="form-group">
                        ...
                        <select name="full_name" class="form-control">
                            @foreach ($arrFullNames as $strFullName)
                                <option value="{{$strFullName}}"  {{isset($_GET['full_name']) && $_GET['full_name']==$strFullName ? "selected" : ""}}>{{$strFullName}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select name="project" class="form-control">
                            @foreach ($arrProjects as $strProject)
                                <option value="{{$strProject}}" {{isset($_GET['project']) && $_GET['project']==$strProject ? "selected" : ""}}>{{$strProject}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="date_range_filter">Date Range</label>
                        <input class="form-control" type="text" name="date_range" id="date_range_filter" value="{{ $_GET['date_range'] ?? "" }}">
                    </div>
                    <div class="form-group">
                        <input type="submit" value="Apply Filter" class="btn btn-primary">
                        <a href="{{route('dashboard.spendings')}}" class="btn btn-danger">Reset Filter</a>
                    </div>
                </form>
            </div>
            @endif
