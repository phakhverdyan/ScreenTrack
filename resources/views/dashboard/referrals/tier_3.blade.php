...
@section('content')
    <section class="container py-5">
        <div class="row">
            <div class="col-md-12">
                <h3 class="[ page-title ]">
                    {!! __('dashboard/referrals.my_referrals', ['count' =>  $count_of_referral_connections->completed / $count_of_referral_connections->total ]) !!}
                </h3>
            </div>
        </div>
        <div class="row">
                                ...
                                @foreach ($referral_connections as $current_referral_connection)
                                    <div data-id="{{ $current_referral_connection->id }}" class="mt-2">
                                        <div class="d-inline-block mr-3 px-0">
                                            ...
                                            <div class="align-self-center d-flex flex-1 mb-0" style="padding-left: 3px;">
                                                <small class="d-flex align-items-center">
                                                    ...
                                                    <img src="{{ asset_no_cache('/img/countries/flags/' . ($current_referral_connection->tier1->referrer_user->country_code ?? '_') . '.png') }}" style="width: 16px;" title="{{ $current_referral_connection->tier1->referrer_user->country->name ?? 'Unknown Country' }}" class="mx-2">
                                                    <b>{{ $current_referral_connection->tier1->referrer_user->short_title }}</b>
                                                    <span class="text-secondary ml-1">
                                                        ● {{ $current_referral_connection->tier1->created_at->diffForHumans() }}
                                                    </span>
                                                </small>
                                            </div>
                                            <div class="align-self-center d-flex flex-1 mb-0" style="padding-left: 100px;">
                                                <small class="d-flex align-items-center">
                                                    <img src="{{ asset_no_cache('img/up-right-arrow.svg') }}" style="width: 16px; height: 16px; transform: rotateZ(-82deg); margin-top: -7px; margin-right: 5px; margin-left: 5px; opacity: 0.3;">
                                                    <span class="text-secondary">Referred by</span>
                                                    <img src="{{ asset_no_cache('/img/countries/flags/' . ($current_referral_connection->tier2->referrer_user->country_code ?? '_') . '.png') }}" style="width: 16px;" title="{{ $current_referral_connection->tier2->referrer_user->country->name ?? 'Unknown Country' }}" class="mx-2">
                                                    <b>{{ $current_referral_connection->tier2->referrer_user->short_title }}</b>
                                                    <span class="text-secondary ml-1">
                                                        ● {{ $current_referral_connection->tier2->created_at->diffForHumans() }}
                                                    </span>
                                                </small>
                                            </div>
                                        </div>
                                        <div class="d-flex flex-1">
                                            ...
                                            <span class="align-self-center ml-3 px-0 text-right text-nowrap">
                                                ...
                                                @else
                                                    <span class="d-inline-block" style="margin-top: -7px; vertical-align: middle;">
                                                        Waiting for first contract
                                                        <img src="{{ asset_no_cache('img/question-circle.svg') }}" style="width: 20px; margin-top: -1px; opacity: 0.3;" data-toggle="tooltip" data-placement="right" title="In order to determine if this is a manager ($150 USD commission) or a freelancer ($100 USD commission) we'll first have to see in what direction the first contract will flow. Once the contract has the first payment, we will display the right amount.">
                                                    </span>
                                                @endif
                                            </span>
                                        </div>
